...
# Start HTML file
html_file = open("reaction_data.html", "w")
...
for i in range(len(elmList)):
  ...
  for j in range(N):
    ...
    if label == '----> MOLECULES involved !!!':
      html_file.write(f"<p style='color:blue; white-space: pre;'>{strz}{spaces}</p>")
    else:
      html_file.write(f"<p style='color:black; white-space: pre;'>{strz}{spaces}</p>")
