...
print(f'\nT1, T2, T3 before adding k (nx + k) = {TEvol[nx-1], TEvol[nx], TEvol[nx+1]}\n')
...
if delta_T >= 0.01:
  ...
  #f_cubic = interp1d(tarr, Tarr, kind='cubic') # When datapoints are low this returns error as it cannot calculate derivative in the boundaries (GPT4).
  #T_interp = f_cubic(tFine)
