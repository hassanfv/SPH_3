...
#===== smooth_BH (non-parallel)
#@njit
def smoothing_BH(rBH, r):
	...
	for i in range(N):
	    ...
	    dy = rBH[0, 1] - r[i, 1]
	    dz = rBH[0, 2] - r[i, 2]
	    dist[i] = (dx**2 + dy**2 + dz**2)**0.5

	return np.sort(dist)[64] * 0.5
...
#===== get Weights BH
#@njit
def InjectEnergy_weights(rBH, r, hBH, h): 
