...
void readBinaryFile(const char* filename, int& N, int& N_ionFrac, int** Typ, float** x, float** y, float** z,
                    float** vx, float** vy, float** vz, float** rho, float** h, float** u, float** mass, float** gradRhoNorm, float** ionFrac)
{
    ...
    *ionFrac = new float[N_ionFrac];
    ...
    file.read(reinterpret_cast<char*>(*gradRhoNorm), N * sizeof(float));
...
int main()
{
  ...
  const char* filename = "G-0.024995.bin"; //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

  float unit_rho = 4.19712e-24; // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  int N, N_ionFrac;
  int* Typ;
  float *x, *y, *z, *vx, *vy, *vz, *rho, *h, *u, *mass, *gradRhoNorm, *ionFrac;

  readBinaryFile(filename, N, N_ionFrac, &Typ, &x, &y, &z, &vx, &vy, &vz, &rho, &h, &u, &mass, &gradRhoNorm, &ionFrac);
