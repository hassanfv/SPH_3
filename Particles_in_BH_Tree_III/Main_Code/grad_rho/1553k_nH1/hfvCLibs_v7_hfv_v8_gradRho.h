...
void saveArraysToBinary(const std::string &filename, float *x, float *y, float *z, float *vx, float *vy, float *vz,
                        float *rho, float *h, float *u, float *mass, float *gradRhoNorm, float *ionFrac, int *Typ, int N, int N_ionFrac)
{
                                  ...
                                  int *ngb, int MAX_ngb, float *h, float *gradRhoNorm, int N)
{
  ...
  if ((i < N) && (Typ[i] == 0))
  {
    ...
    gradRhoNorm[i] = gradrhox * gradrhox + gradrhoy * gradrhoy + gradrhoz * gradrhoz;
  }
}
