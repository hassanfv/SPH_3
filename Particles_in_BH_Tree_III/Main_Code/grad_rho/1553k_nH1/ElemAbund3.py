...
filename = './Outputs/G-0.024995.bin'
...
def readBinaryFile(filename):
    with open(filename, "rb") as file:
        ...
        gradRhoNorm = np.fromfile(file, dtype=np.float32, count=N)
        ionFrac = np.fromfile(file, dtype=np.float32, count=N_ionFrac)

    return N, N_ionFrac, Typ, x, y, z, vx, vy, vz, rho, h, u, mass, gradRhoNorm, ionFrac
...
N, N_ionFrac, Typ, x, y, z, vx, vy, vz, rho, h, u, mass, gradRhoNorm, ionFrac = readBinaryFile(filename)
...
clump = clumps[1] # ----> the first clump !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
plt.hist(nH_cgs_clump, bins = 100)
plt.show()
...
print(f'For particle with max nH we have Temp = {Temp[nt]}')
...
rtmp = ((xtmp - x[nt]) * (xtmp - x[nt]) + (ytmp - y[nt]) * (ytmp - y[nt]) + (ztmp - z[nt]) * (ztmp - z[nt]))**0.5


'''
hmax = h[nt] # hmax is h for the particle with max(nH)
print('hmax = ', hmax)

res = []
for i in range(40):
  Rc = i * 0.1 * hmax + 2.0 * hmax
  Nc = len(np.where(rtmp <= Rc)[0])
...
plt.scatter(Rc, Nc)
plt.axvline(x = 4.0*hmax, linestyle = '--', color = 'b')
plt.show()
'''
...
plt.scatter(xtmp, ytmp, s = 1, color = 'k')
plt.scatter(xt, yt, color = 'red', s = 10) #----- Line of Sight ---------
plt.plot(xt, yt, color = 'lime') #----- Line of Sight ---------

# Add a circle
circle = patches.Circle((x[nt], y[nt]), 2.0*hmax, color='blue', fill=False)  # Create a circle around (xt, yt) with radius Rc
plt.gca().add_patch(circle)  # Add the circle to the current axes

circle = patches.Circle((x[nt], y[nt]), 4.0*hmax, color='blue', fill=False)  # Create a circle around (xt, yt) with radius Rc
plt.gca().add_patch(circle)  # Add the circle to the current axes
