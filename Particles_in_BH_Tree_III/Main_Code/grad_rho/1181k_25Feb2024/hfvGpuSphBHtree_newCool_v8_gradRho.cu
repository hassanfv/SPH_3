...
int main()
{
  ...
  const float XH = 0.7;
  const float kB = 1.3807e-16;
  const float mH = 1.673534e-24;
  ...
  cudaMalloc(&d_NHtot, N * sizeof(float));
  ...
  cudaMemcpy(d_NHtot, NHtot, N * sizeof(float), cudaMemcpyHostToDevice);
  ...
  while (t < tEnd)
  {
    ...
    */
    ...
    float NHtot_tmp = 0.0, h_inter = 0.0, nH_tmp = 0.0;
    ...
    for (int i = 0; i < N; i++)
    {
      ...
      NHtot_tmp = 0.5 * (h_inter + rho[i] / gradRhoNorm[i]) * unitLength_in_cm * nH_tmp;
      NHtot[i] = log10(NHtot_tmp);
    }
    ...
    doCooling<<<gridSize, blockSize>>>(d_Typ, d_x, d_y, d_z, d_rho, d_u, d_NHtot, d_nHGrid, d_TGrid, d_rGrid, d_NHGrid, d_uarr, d_muarr,
                                       d_Heat, d_Cool, N_nH, N_T, N_r, N_NH, gammah, kB, mH, nHLowBound, nHUpBound, TLowBound, TUpBound,
                                       rLowBound, rUpBound, NHLowBound, NHUpBound, dt, unitTime_in_s, UnitDensity_in_cgs,
                                       unitLength_in_cm, Unit_u_in_cgs, N);
    cudaDeviceSynchronize();
    ...
    //------------ SAVING SNAP-SHOTS ------------
    cudaMemcpy(h, d_h, N * sizeof(float), cudaMemcpyDeviceToHost); // Moved outside so that it can be used by nSplit calculator in each time step.
    if (!(counter % 400))
    //if (counter > -1)
    {
      cudaMemcpy(Typ, d_Typ, N * sizeof(int), cudaMemcpyDeviceToHost);
      ...
      saveArraysToBinary(filename, x, y, z, vx, vy, vz, rho, h, u, mass, gradRhoNorm, Typ, N);
    }
