...
filename = './Outputs__/G-0.000000.bin'
...
print('sort (rho/gradRhoNorm) = ', np.sort(rho/gradRhoNorm))
...
plt.scatter(r, rho/gradRhoNorm*unitLength/3.086e18, s = 0.01, color = 'k')
plt.yscale('log')
plt.show()

# Calculate the 90th percentile value of rho
percentile_90_value = np.percentile(rho, 99.99)
...
print()
print(f'h (in pc) = {(h[nt]*unitLength/3.086e18):.3f}   its log(N) = {np.log10(nH * h[nt]*unitLength):.3f}')
print(f'rho/gradRho (in pc) = {(rho[nt]/gradRhoNorm[nt]*unitLength/3.086e18):.3f}    its log(N) = {np.log10(nH*(rho[nt]/gradRhoNorm[nt]*unitLength)):.3f}')
