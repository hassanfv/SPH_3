...
//===== checkAndWriteToFile
void assessMent(int* h_child, int numNodes, const std::string& filenameS) {
  int num_ints = 8 * numNodes;
  ...
  for (int i = num_ints - 1; i >= 0; i--)
  {
    ...
    {
      lastIndex = i;
      break; // Break the loop as soon as you find a non -1 value
    }
  }

  std::ofstream file(filenameS, std::ios::app); // Open file in append mode
  ...
  file << "len(h_child) = " << num_ints << std::endl;

  if (lastIndex != -1)
  {
    file << "Last index where h_child is not -1: " << lastIndex << std::endl;
  }
  else
  {
    file << "All elements are -1." << std::endl;
  }

  if (lastIndex > static_cast<int>(num_ints - 0.1f * num_ints))
  {
    file << "\n!!!!!!!!!!!!!!! DANGER DANGER DANGER DANGER DANGER DANGER !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n";
    file << "!!!!!!!! It seems the h_child (and d_child) size is not enough. Try to increase it !!!!!!" << std::endl;
    file << "!!!!!!!!!!!!!!! DANGER DANGER DANGER DANGER DANGER DANGER !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n\n";
  }
...
void assessMent2(float* h_x, int numNodes, const std::string& filenameS)
{
  // Assuming the condition is to find the last index where the value is not 0.0
  float conditionValue = 0.0f;

  // Find the last index where h_x does not meet the condition
  int lastIndex = -1; // Initialize to -1 to indicate not found

  for (int i = numNodes - 1; i >= 0; i--)
  {
    if (h_x[i] != conditionValue)
    {
      lastIndex = i;
      break; // Break the loop as soon as you find a non-condition value
    }
  }
  ...
  if (lastIndex != -1)
  {
    file << "Last index where h_x does not equal " << conditionValue << ": " << lastIndex << std::endl;
  }
  else
  {
    file << "All elements equal " << conditionValue << "." << std::endl;
  }

  if (lastIndex > static_cast<int>(numNodes - 0.1f * numNodes))
  {
    file << "\n!!!!!!!!!!!!!!! DANGER DANGER DANGER DANGER DANGER DANGER !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n";
    file << "!!!!!!!! It seems you need to increase the value of numNodes as h_x (and dev_x) size is not enough !!!!!!" << std::endl;
    file << "!!!!!!!!!!!!!!! DANGER DANGER DANGER DANGER DANGER DANGER !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n\n";
  }
...
int main()
{
  ...
  float dt = 2e-6; //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! This is only the first time step !!
  ...
  //string filenameS = "AssessMent.txt";
  
  //*******************************************************************
  //******************* Reading Cooling File **************************
  //*******************************************************************
  ifstream file("coolHeatGridDec2023.bin", ios::binary);

  if (!file) {
    cerr << "Failed to open coolHeatGridDec2023.bin file." << endl;
    return 1;
  }
...
//------------- Just for testing ---------
  ...
  int indx = jjj * (N_T * N_nH * N_Z) + i * (N_nH * N_Z) + j * (N_Z) + k;
  
  int ii_HI  = 0;
  int ii_HII = 1;
  int indx_HI  = jjj * (N_T * N_nH * N_Z * N_M) + i * (N_nH * N_Z * N_M) + j * (N_Z * N_M) + k * (N_M) + ii_HI;
  int indx_HII = jjj * (N_T * N_nH * N_Z * N_M) + i * (N_nH * N_Z * N_M) + j * (N_Z * N_M) + k * (N_M) + ii_HII;
  
  cout << "delta_u = " << uEvolutionX[indx] << endl;
  cout << "mu = " << muArrX[indx] << endl;
  cout << "HI fraction = " << metalzX[indx_HI] << endl;
  cout << "HII fraction = " << metalzX[indx_HII] << endl;
//--------------------------------------------
  
  float *Temp, *d_Temp, *uArr, *d_uArr, *nH, *d_nH, *Z, *d_Z, *HCool, *d_HCool, *muA, *d_muA, *kpc, *d_kpc, *metalz, *d_metalz;
  
  int N_HCool = N_kpc * N_T * N_nH * N_Z;
  int N_metalz = N_kpc * N_T * N_nH * N_Z * N_M;
  ...
  cudaMalloc(&d_uArr, N_HCool * sizeof(float));
  ...
  cudaMemcpy(d_uArr, uArr, N_HCool * sizeof(float), cudaMemcpyHostToDevice);
  ...
  //------- For ngb_new -----
  int nSplit = 30; //32; // for 3D, never go above 10 as 10^3 = 1000 and the maximum blockSize in GPUs is 1024!! IMPORTANT !!
  ...
  assessMent(h_child, numNodes, filenameS);
  //-----------------------------------------------------------------------------------
  ...
  */
  ...
  /*
  //------ Writing dev_x ---------------------------------
  auto T_assess_2 = std::chrono::high_resolution_clock::now();
  //-------- Used for initial assessment to check if "numNodes" is big enough ---------
  cudaMemcpy(h_x, dev_x, numNodes * sizeof(float), cudaMemcpyDeviceToHost);
  assessMent2(h_x, numNodes, filenameS);
  //-----------------------------------------------------------------------------------
  auto end_assess_2 = std::chrono::high_resolution_clock::now();
  auto elapsed_assess_2 = std::chrono::duration_cast<std::chrono::nanoseconds>(end_assess_2 - T_assess_2);
  cout << "T_assess_2 = " << elapsed_assess_2.count() * 1e-9 << endl;
  //------------------------------------------------------
  */
  ...
  while (t < tEnd)
  {
    ...
    cudaMalloc(&d_ngbDebug, N * sizeof(int));
    cudaMemset(d_ngbDebug, -1, N * sizeof(int));
    ...
    */
    ...
    if (!(counter % 1000))
    {
      T_assess_1 = std::chrono::high_resolution_clock::now();
      //-------- Used for initial assessment to check if "numNodes" is big enough ---------
      cudaMemcpy(h_child, d_child, 8 * numNodes * sizeof(int), cudaMemcpyDeviceToHost);
      assessMent(h_child, numNodes, filenameS);
      //-----------------------------------------------------------------------------------
      end_assess_1 = std::chrono::high_resolution_clock::now();
      elapsed_assess_1 = std::chrono::duration_cast<std::chrono::nanoseconds>(end_assess_1 - T_assess_1);
      cout << "T_assess_1 = " << elapsed_assess_1.count() * 1e-9 << endl;
    }
    */
    ...
    {
      //------------------------------------------------------
      T_assess_2 = std::chrono::high_resolution_clock::now();
      //-------- Used for initial assessment to check if "numNodes" is big enough ---------
      cudaMemcpy(h_x, dev_x, numNodes * sizeof(float), cudaMemcpyDeviceToHost);
      assessMent2(h_x, numNodes, filenameS);
      //-----------------------------------------------------------------------------------
      end_assess_2 = std::chrono::high_resolution_clock::now();
      elapsed_assess_2 = std::chrono::duration_cast<std::chrono::nanoseconds>(end_assess_2 - T_assess_2);
      cout << "T_assess_2 = " << elapsed_assess_2.count() * 1e-9 << endl;
      //------------------------------------------------------
    }
    */
    ...
    hcoolingZ<<<gridSize, blockSize>>>(d_Typ, d_u, d_uArr, d_rho, d_metalz, Zmetal, dt, // Zmetal is the gass metallicity assumed.
                                       d_nH, d_Z, d_HCool, d_ionFrac, d_x, d_y, d_z,
                                       d_muA, d_Temp, d_kpc, UnitDensity_in_cgs, unitTime_in_s, 
                                       Unit_u_in_cgs, unitLength_in_cm, kpc_in_cm, GAMMA_MINUS1,
                                       N_kpc, N_nH, N_Z, N_T, N_M, N);
    cudaDeviceSynchronize();
