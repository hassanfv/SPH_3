...
void saveArraysToBinary(const std::string &filename, float *x, float *y, float *z, float *vx, float *vy, float *vz,
                        float *rho, float *h, float *u, float *mass, float *ionFrac, int *Typ, int N, int N_ionFrac, int *ngbDebug)
{
  ...
  file.write(reinterpret_cast<const char *>(ngbDebug), N * sizeof(int));
...
__global__ void smoothing_h_ngb(int *Typ, float *x, float *y, float *z, float *h,
                                int N, int Ndown, int Nup, float coeff,
                                float Nngb_f, float *Nngb_previous, float *divV, int *ngb, int MAX_ngb, int *ngbDebug, float dt)
{
  ...
  if ((i < N) && (Typ[i] == 0))
  {
    ...
    ngbDebug[i] = jjCounter; // For checking to confirm that we never reach MAX_ngb. The lower band will always be > 65, see ngbDB_v2 !
...
//=====================================================================================
//======================= Heating, Cooling using CHIMES =============================== (27 Dec 2023)
//=====================================================================================
__global__ void hcoolingZ(int *Typ, float *u, float *uArr, float *rho, float *metalz, float Zmetal, float dt, // Zmetal is the gas metallicity assumed.
                         ...
                         int N_kpc, int N_nH, int N_Z, int N_T, int N_M, int N)
{
  ...
  if ((i < N) && (Typ[i] == 0))
  {
    ...
    //float U[N_T]; // Array defined in each thread!
    
    // Dynamically allocate the array U
    //float* U = (float*)malloc(N_T * sizeof(float));
    //if (U == NULL) return; // Check for successful allocation
    ...
    int stride_kpc = N_T * N_nH * N_Z;
    int stride_T = N_nH * N_Z;
    int stride_nH = N_Z;
    
    float u_i = u[i] * unit_u_in_cgs;
    
    // We loop over N_T! We use U to find ndx_u!
    /*
    for (int t = 0; t < N_T; t++)
    {
      int index = (ndx_kpc * stride_kpc) + (t * stride_T) + (ndx_nH * stride_nH) + ndx_Z;
      U[t] = uArr[index];
    }
    */
    ...
    for (int j = 0; j < N_T; j++)
    {
      index = (ndx_kpc * stride_kpc) + (j * stride_T) + (ndx_nH * stride_nH) + ndx_Z;
    ...
    int ndx = (ndx_kpc * stride_kpc) + (ndx_u * stride_T) + (ndx_nH * stride_nH) + ndx_Z;
    
    //---------------------
    // Calculate the strides for Metalz
    int stride_kpcx = N_T * N_nH * N_Z * N_M;
    int stride_Tx = N_nH * N_Z * N_M;
    int stride_nHx = N_Z * N_M;
    int stride_Zx = N_M;
    
    if (ndx_u > 0)
    {
      int ndx_minus1 = (ndx_kpc * stride_kpc) + ((ndx_u-1) * stride_T) + (ndx_nH * stride_nH) + ndx_Z;
      float uEv_1 = uEvol[ndx_minus1];
      float uEv_2 = uEvol[ndx];
      
      int index_U = (ndx_kpc * stride_kpc) + (ndx_u * stride_T) + (ndx_nH * stride_nH) + ndx_Z;
      int index_U_minus1 = (ndx_kpc * stride_kpc) + ((ndx_u - 1) * stride_T) + (ndx_nH * stride_nH) + ndx_Z;
    ...
    float dt_in_year = dt * unit_time_in_sec / 3600.0f / 24.0f / 365.25f;
    
    float new_u_i = u_i + (uEv * dt_in_year); // Note that uEv is the change in u in one year!!! Also u_i is already multiplied by unit_u_in_cgs above!
    
    //if (new_u_i < 0.0)
    //if (i == 983651)
    if (new_u_i < 0.0)
      printf("NEGATIVE u &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&==> i, u_i, new_u_i, x[i], y[i], z[i] = %d, %f, %f %f, %f, %f\n", i, u_i/unit_u_in_cgs, new_u_i/unit_u_in_cgs, x[i], y[i], z[i]);
    
    u[i] = new_u_i / unit_u_in_cgs; // new_u_i is the u after heating and/or cooling!
    
    for (int k = 0; k < N_M; k++)
    {
      int ndx_M = (ndx_kpc * stride_kpcx) + (ndx_u * stride_Tx) + (ndx_nH * stride_nHx) + (ndx_Z * stride_Zx) + k;
...
//=====================================================================================
__global__ void hcoolingxXXXX(int *Typ, float *u, float *U, float *rho, float *metalz, float Zmetal, float dt, // Zmetal is the gass metallicity assumed.
