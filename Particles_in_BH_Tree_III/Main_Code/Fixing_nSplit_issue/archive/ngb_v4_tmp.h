...
//================ ngbDB_new_v2 =================
//===============================================
__global__ void ngbDB_new_v2(int *Typ, float *x, float *y, float *z, float *h,
                             ...
                             int *ngb, int MAX_ngb, int N, int *flag)
{
  ...
  if ((i < N) && (Typ[i] == 0))
  {
    ...
    int debugger = 50; // set to 1000 so that it can satisfy the first while loop condition !
    int Lim = 1;
    
    while ((debugger >= 50) && (Lim <= 2))
    {
      //--------- Finding the neighboring cells -------------
      int neighbors[125]; // highly likely will stay in the register of the thread!
      ...
      while ((k < 70 || k > 190) && (debugger < 50))
      {
        ...
        //if (Lim == 2)
        //  printf("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! i, k, Lim, x, y, z, DEBUGGER = %d, %d, %d, %f, %f, %f, %d\n", i, k, Lim, x[i], y[i], z[i], debugger);
      ...
      //if (isnan(x[i]))
      //  printf("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! i, Type, x, y, z, DEBUGGER = %d, %d, %f, %f, %f, %d\n", i, Typ[i], x[i], y[i], z[i], debugger);
        
      //if (debugger > 1)
      //  printf("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! i, Type, x, y, z, DEBUGGER = %d, %d, %f, %f, %f, %d\n", i, Typ[i], x[i], y[i], z[i], debugger);
      Lim = 2; // This will be activated in cases where the particle distribution is so sparse that 27 neighboring cells do not contain at least 70 particles!
    }
  }
}
