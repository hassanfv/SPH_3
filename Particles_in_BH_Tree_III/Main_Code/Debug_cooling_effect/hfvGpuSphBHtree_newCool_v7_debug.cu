...
#include <random>
#include <tuple>
#include "hfvCLibs_v7_hfv_v8_debug.h"
...
int main()
{
  ...
  //*******************************************************************
  ifstream file("coolHeatGridJan2024.bin", ios::binary);

  if (!file) {
    cerr << "Failed to open coolHeatGridJan2024.bin file." << endl;
    return 1;
  }
  ...
  float *Temp, *d_Temp, *uArr, *d_uArr, *nH, *d_nH, *Z, *d_Z, *HCool, *d_HCool, *muA, *d_muA, *kpc, *d_kpc, *metalz, *d_metalz, *tArr, *d_tArr;
  ...
  cudaMemcpy(d_tArr, tArr, N_t * sizeof(float), cudaMemcpyHostToDevice);
  ...
  cudaMemcpy(d_uArr, uArr, (N_kpc * N_T * N_nH * N_Z) * sizeof(float), cudaMemcpyHostToDevice);
  ...
  cudaMalloc(&d_uAad, N * sizeof(float));
  ...
  cudaMemcpy(d_uAad, uAad, N * sizeof(float), cudaMemcpyHostToDevice);
  ...
  while (t < tEnd)
  {
    ...
    */
    ...
    //&&&&&&&&&&&&&&&&&&& u_after_adiabatic &&&&&&&&&&&&&&&&&&&&&&&&
    u_after_adiabatic<<<gridSize, blockSize>>>(d_Typ, d_u, d_uAad, N);
    cudaDeviceSynchronize();
    ...
    hcoolingZ<<<gridSize, blockSize>>>(d_Typ, d_u, d_uArr, d_rho, d_metalz, Zmetal, dt, // Zmetal is the gass metallicity assumed.
                                       ...
                                       d_tArr, N_kpc, N_nH, N_Z, N_T, N_M, N_t, N);
    cudaDeviceSynchronize();
    ...
    //if (counter > -1)
    {
      ...
      cudaMemcpy(uAad, d_uAad, N * sizeof(float), cudaMemcpyDeviceToHost);
      ...
      saveArraysToBinary(filename, x, y, z, vx, vy, vz, rho, h, dudt, dudt_pre, uBad, uAad, u, mass, Typ, N);
    }
