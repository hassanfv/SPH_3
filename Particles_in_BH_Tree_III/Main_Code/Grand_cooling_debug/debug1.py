...
for i, filename in enumerate(filez):
  ...
  rho_cgs = rho[jj] * unit_rho
  XH = 0.7
  nH = rho_cgs * XH / mH
  print(f'{format(t[i], ".7f")}, {format(nH, ".3f")},     {format(uB_ad[jj], ".3f")}, {format(uA_ad[jj], ".3f")}, {format(uA_ad[jj] - uB_ad[jj], ".3f")},     {format(uB_hc[jj], ".3f")}, {format(uA_hc[jj], ".3f")}, {format(uA_hc[jj] - uB_hc[jj], ".3f")}')
  
  print(f'rho[i] = {rho[jj]}; u[i] = {uB_hc[jj]}; x[i] = {x[jj]}; y[i] = {y[jj]}; z[i] = {z[jj]};')
  print()
