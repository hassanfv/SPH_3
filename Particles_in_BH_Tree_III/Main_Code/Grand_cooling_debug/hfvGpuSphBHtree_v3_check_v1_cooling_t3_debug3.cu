...
//--------------------------------------------
  ...
  float *uB, *d_uB, *uA, *d_uA, *uAC, *d_uAC;
  ...
  cudaMalloc(&d_uB, N * sizeof(float));
  cudaMalloc(&d_uA, N * sizeof(float));
  ...
  cudaMemcpy(d_uB, uB, N * sizeof(float), cudaMemcpyHostToDevice);
  cudaMemcpy(d_uA, uA, N * sizeof(float), cudaMemcpyHostToDevice);
  ...
  //------- For ngb_new -----
  int nSplit = 24; // for 3D, never go above 10 as 10^3 = 1000 and the maximum blockSize in GPUs is 1024!! IMPORTANT !!
  ...
  while (t < tEnd)
  {
    ...
    */
    ...
    //&&&&&&&&&&&&&&&&&&& u BEFOR Adiabatic &&&&&&&&&&&&&&&&
    u_before_adiabatic<<<gridSize, blockSize>>>(d_Typ, d_u, d_uB, N);
    ...
    //&&&&&&&&&&&&&&&&&&& u AFTER Adiabatic &&&&&&&&&&&&&&&&
    u_before_adiabatic<<<gridSize, blockSize>>>(d_Typ, d_u, d_uA, N);
    ...
    //&&&&&&&&&&&&&&&&&&& u AFTER Adiabatic &&&&&&&&&&&&&&&& ===> uAC is the same as u
    //u_before_adiabatic<<<gridSize, blockSize>>>(d_Typ, d_u, d_uAC, N);
    ...
    //if (counter > -1)
    {
      ...
      cudaMemcpy(uB, d_uB, N * sizeof(float), cudaMemcpyDeviceToHost);
      cudaMemcpy(uA, d_uA, N * sizeof(float), cudaMemcpyDeviceToHost);
      ...
      saveArraysToBinaryX2(filename, x, y, z, vx, vy, vz, rho, h, uB, uA, u, dudt, mass, Typ, N);
    }
