...
jj = 857055   # [857055 862834 929998 933578] # [874002 875985 877706 877710 939445 940947]
...
for i, filename in enumerate(filez):
  ...
  res.append([i,np.log10(Temp[jj]),rho[jj],h[jj],vr[jj]*unit_velocity_cgs/100000, uB_ad[jj], uA_ad[jj], uB_hc[jj], uA_hc[jj],dudt_ad[jj],dudt_hc[jj],dudt[jj]])
  
  ntmp = np.where((Temp > 6000000) & (Temp < 6500000) & (x > 0.0) & (y > 0.0) & (np.abs(z) < 0.035))[0]
  print(i, uB_ad[jj], uA_ad[jj], uB_hc[jj], uA_hc[jj], dudt_ad[jj], dudt_hc[jj], dudt[jj])
  print('ntmp = ', ntmp)
  print()
...
axs[1, 3].plot(t, dudt_ad, label='dudt_ad', color='red')
axs[1, 3].plot(t, dudt, label='dudt_add_orig', color='blue')
axs[1, 3].plot(t, dudt_hc, label='dudt_hc', color='orange')
axs[1, 3].plot(t, np.abs(dudt_hc), label='abs(dudt_hc)', color='lime')
