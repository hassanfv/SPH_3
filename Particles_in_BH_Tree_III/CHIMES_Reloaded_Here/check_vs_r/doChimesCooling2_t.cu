...
{
  float dx = (log10(u_p) - log10(uGrid[nxu0])) / (log10(uGrid[nxu1]) - log10(uGrid[nxu0]));  //!! CHECK if we use log(u), what happens in accuracy!!!!!!
    ...
    for (int j = nxnH0; j <= nxnH1; j++)
    {
      for (int k = nxr0; k <= nxr1; k++)
      {
        for (int l = nxNH0; l <= nxNH1; l++)
        {
          ...
          {
            pData[nxs] = log10(uarr[i * S1 + j * S2 + k * S3 + l * S4 + m]);
            nxs += 1;
            printf("PG = %e\n", (uarr[i * S1 + j * S2 + k * S3 + l * S4 + m]));
          }
        }
      }
    }
  ...
  return powf(10, uFinal); // Use powf(10, LamC) for floating-point exponentiation
}
...
int main()
{
  ...
  doCooling<<<1, Npart>>>(d_Typ, d_x, d_y, d_z, d_rho, d_u_p, d_NH,
  ...
  cout << "u After = " << u_p[0]*unit_u_in_cgs << endl << endl;

  for (int i = 0; i < Npart; i++)
  {
    cout << "r_p, uB, uA = " << rr_p[i] << ", " << uB[i] << ", " << u_p[i] << endl;
  }
