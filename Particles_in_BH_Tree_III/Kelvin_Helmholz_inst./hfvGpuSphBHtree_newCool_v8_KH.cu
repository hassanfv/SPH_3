...
int main()
{
  ...
  std::ifstream file("IC_KH_1000k.bin", std::ios::binary);
  if (!file.is_open()) {
      // Handle error
      return -1;
  }
  ...
  cudaError_t err = cudaMemcpy(u, d_u, N * sizeof(float), cudaMemcpyDeviceToHost);
  if (err != cudaSuccess) {
  ...
  // Copy data from device to host and check for errors
  err = cudaMemcpy(ngb, d_ngb, N * MAX_ngb * sizeof(float), cudaMemcpyDeviceToHost);
  if (err != cudaSuccess) {
