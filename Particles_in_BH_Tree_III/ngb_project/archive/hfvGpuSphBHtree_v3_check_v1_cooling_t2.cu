...
//--------------------------------------------
  ...
  */
  ...
  std::ofstream file2("h_x.bin", std::ios::out | std::ios::binary);
  if (!file2)
  {
      std::cerr << "Cannot open h_x.bin file for writing." << std::endl;
  }
  file2.write(reinterpret_cast<char*>(h_x), numNodes * sizeof(float));
  file2.close();
  cout << "h_x.bin assessment file created !!!!" << endl;
  //------------------------------------------------------
