    ...
    {  
        ...
        for(int i = 0; i < 8; i++)
        {
            if (bodyIndex + offset != 0)
              child[(bodyIndex + offset) * 8 + i - hShift] = -1; // iterates over the 8 child nodes! -1 indicates the node initially has no children!
            else
  ...
  int hShift = 8 * n - 10;
	...
	while((bodyIndex + offset) < n)
	{
      ...
      else
      {
		...
		if (temp != 0)
  		childIndex = child[temp * 8 + childPath - hShift];
    else
      childIndex = child[temp * 8 + childPath];
      ...
      else
      {
          f = 0.5f*(f+ba);
			...
			if (temp != 0)
  			childIndex = child[8 * temp + childPath - hShift];
  		else
		...
		{
		  if (temp != 0)
			  locked = temp * 8 + childPath - hShift;
			else
			  locked = temp * 8 + childPath
			...
			if(atomicCAS(&child[locked], childIndex, -2) == childIndex)
			{
				...
				else
				{
					...
					while(childIndex >= 0 && childIndex < n)
					{
				 		...
				 		{
				 		  if (temp != 0)
				 			  child[8 * temp + childPath - hShift] = cell;
				 			else
            ...
            else
            {
              f = 0.5f*(f+ba);
				 		...
				 		if (cell != 0)
				 		  child[8 * cell + childPath - hShift] = childIndex;
				 		else
            ...
            else
            {
                f = 0.5f*(f+ba);
				  ...
				  if (temp != 0)
				 	  child[8 * temp + childPath - hShift] = bodyIndex + offset;
				 	else
  ...
  int node = 0;
	...
	while((cell + offset) < ind)
	{
		...
		if(s >= 0)
		{
			...
			{
			  if ((cell+offset) != 0)
				  node = child[8 * (cell+offset) + i - hShift];
				else
				  node = child[8 * (cell+offset) + i];
