...
def readArraysFromBinary(filename):
    with open(filename, 'rb') as file:
        ...
        # Read Ncell2
        Ncell2 = np.fromfile(file, dtype=np.int32, count=1)[0]
        ...
        # Read countx using Ncell2 for count
        countx = np.fromfile(file, dtype=np.int32, count=Ncell2)

    return x, y, z, vx, vy, vz, rho, h, u, groupedIndex, Typ, Ncell2, countx

# Usage
x, y, z, vx, vy, vz, rho, h, u, groupedIndex, Typ, Ncell2, countx = readArraysFromBinary(filename)
...
rtmp = np.sqrt((x - x[nnt])**2 + (y - y[nnt])**2 + (z - z[nnt])**2)

print(np.sort(rtmp))

plt.scatter(x, y, s = 0.01)
plt.scatter(x[nnt], y[nnt], s = 2.0, color = 'red')
plt.show()
