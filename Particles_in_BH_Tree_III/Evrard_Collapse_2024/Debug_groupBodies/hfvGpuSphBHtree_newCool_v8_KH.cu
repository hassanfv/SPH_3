...
#include <random>
#include <tuple>
#include "hfvCLibs_v7_hfv_v8.1.1.h"
...
int main()
{
  ...
  std::ifstream file("IC_Evrard24_394k.bin", std::ios::binary);
  if (!file.is_open()) {
      // Handle error
      return -1;
  }
  ...
  */
  ...
  while (t < tEnd)
  {
    ...
    //r_evolveX<<<gridSize, blockSize>>>(d_Typ, d_x, d_y, d_z, d_vx, d_vy, d_vz, dt, N, maxRange);
    ...
    cout << *h_leftX << ", " << *h_rightX << ", " << *h_bottomX << ", " << *h_topX << ", " << *h_frontX << ", " << *h_backX << endl;
    cout << x[197209] << ", " << y[197209] << ", " << z[197209] << endl; 
    ...
    //if (counter > -1)
    {
      ...
      saveArraysToBinary(filename, x, y, z, vx, vy, vz, rho, h, u, groupedIndex, countx, Typ, N, 2*Ncell);
    }
