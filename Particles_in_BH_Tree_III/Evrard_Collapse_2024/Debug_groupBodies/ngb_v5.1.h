                             ...
                             int *ngb, int MAX_ngb, int N, int *flag)
{
  ...
  if ((i < N) && (Typ[i] == 0))
  {
    ...
    while (debugger >= 50)
    {
      ...
      int neighbors[3375]; // highly likely will stay in the register of the thread!
        ...
        for (int q = 0; q < count; q++)
        {
          ...
          while (jj < ending_nx && k < MAX_ngb)
          {
            ...
            if (Typ[j] == 0)
            {
              ...
              if (i == -197209)
              {
                printf("i, k, h_new, x[i], y[i], z[i], x[j], y[j], z[j], rr, cell_id, LIM = %d, %d, %f, %f, %f, %f, %f, %f, %f, %f, %d, %d\n",
                        i, k, h_new, x_i,  y_i,  z_i,  x[j], y[j], z[j], rr, cell_id, Lim);
              }
      ...
      if (Lim == 7 || i == -197209)
      {
        ...
        printf("!!!!!!!!!!!!!! LIM LIM ===> i, k, h_new, x[i], y[i], z[i], r, LIM = %d, %d, %f, %f, %f, %f, %f, %d\n", i, k, h_new, x_i, y_i, z_i, rttt, Lim);
      }
      
      if ((debugger >= 50) && (Lim == 7))
      {
        *flag = 1; //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        return;
      }
