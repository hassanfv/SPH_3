...
void saveArraysToBinary(const std::string &filename, float *x, float *y, float *z, float *vx, float *vy, float *vz,
                        float *rho, float *h, float *u, int *groupedIndex, int *countx, int *Typ, int N, int Ncell2)
{
  ...
  file.write(reinterpret_cast<const char *>(&Ncell2), sizeof(int));
  ...
  file.write(reinterpret_cast<const char *>(groupedIndex), N * sizeof(int));
  
  // Write the countx array to the file
  file.write(reinterpret_cast<const char *>(countx), Ncell2 * sizeof(int));
...
//================= velocity evolution ======================
//===========================================================
__global__ void v_evolve_positive(int *Typ, float *x, float *y, float *z, float *vx, float *vy, float *vz,
                         float *accx, float *accy, float *accz,
                         float dt, int N)
{
  ...
  if ((i < N) && ((Typ[i] == 0) || (Typ[i] == 1)))
  {
    ...
    float vtmp = (vx[i] * x[i] + vy[i] * y[i] + vz[i] * z[i]) / sqrt(x[i] * x[i] + y[i] * y[i] + z[i] * z[i]);
...
//================= position evolution ======================
//===========================================================
__global__ void r_evolveX(int *Typ, float *x, float *y, float *z,
                         float *vx, float *vy, float *vz,
                         float dt, int N, float maxRange)
{
  ...
  if ((i < N) && ((Typ[i] == 0) || (Typ[i] == 1)))
  {
    ...
    if (x[i] > maxRange)
      x[i] = maxRange;
    if (x[i] < -1.0f * maxRange)
      x[i] = -1.0f * maxRange;
    
    if (y[i] > maxRange)
      y[i] = maxRange;
    if (y[i] < -1.0f * maxRange)
      y[i] = -1.0f * maxRange;
    
    if (z[i] > maxRange)
      z[i] = maxRange;
    if (z[i] < -1.0f * maxRange)
      z[i] = -1.0f * maxRange;
