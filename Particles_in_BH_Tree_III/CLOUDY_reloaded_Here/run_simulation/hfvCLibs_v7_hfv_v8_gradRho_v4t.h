                                 ...
                                 float dt, int N)
{
  ...
  */
  ...
  if ((i < N) && (Typ[i] == 0))
  {
    ...
    float rhocgs = rho[i] * UnitDensity_in_cgs;
    float nH_p = log10(rhocgs * XH / mH); // Note: we take log10!
    float r_p = sqrt(x[i] * x[i] + y[i] * y[i] + z[i] * z[i]);
    float NH_p = NH[i];
    float u_p = u[i] * Unit_u_in_cgs;
    ...
    float u_tmp = u[i] * Unit_u_in_cgs + Gam_minus_Lam / rhocgs * dt_sec;// Note: since u_p is log10(u_p) here we use directly "u[i] * Unit_u_in_cgs" !
    ...
    */
    ...
    u[i] = u_tmp / Unit_u_in_cgs; // converting back to code unit!
  }
}
