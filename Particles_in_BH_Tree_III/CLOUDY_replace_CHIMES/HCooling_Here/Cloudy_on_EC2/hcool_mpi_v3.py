...
# mu added to dfIonFrac (22 Feb. 2024)
...
def mainFunc(nbeg, nend, RiD):
  ...
  for i in range(nbeg, nend):
    ...
    resIon.append([np.round(iList[0], 4), np.round(iList[1], 4), np.round(iList[2], 4), np.round(iList[3], 4)] + iFrac + [np.round(mu, 4)])
