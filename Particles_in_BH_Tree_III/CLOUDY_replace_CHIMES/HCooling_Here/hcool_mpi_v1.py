...
Xres = 0 # This is just a placeholder. Its absence would crash this line--> Xres = comm.bcast(Xres, root = 0) for CPUs with rank != 0.

if rank == 0:
	Xres = local_res
	for i in range(1, nCPUs):
		res_tmp = comm.recv(source = i)
		Xres = np.concatenate((Xres, res_tmp), axis=0)
else:
	comm.send(local_res, dest = 0)

Xres = comm.bcast(Xres, root = 0)
#----------------------------
