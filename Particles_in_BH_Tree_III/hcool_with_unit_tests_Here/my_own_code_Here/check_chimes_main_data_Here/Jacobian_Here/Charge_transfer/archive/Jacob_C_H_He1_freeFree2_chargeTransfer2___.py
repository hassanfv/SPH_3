...
def Lambda(T, nH0, nHp, nHe0, nHep, nHepp, nC0, nC1, nC2, nC3, nC4, nC5, nC6):
    ...
    spec = [nH0, nHp, nHe0, nHep, nHepp, nC0, nC1, nC2, nC3, nC4, nC5, nC6]
    Zspec= [0.,  1.,   0.,   1.,   2.,    0.,  1.,  2.,  3.,  4.,  5.,  6.]
    
    LFF = 0.0
    for i in range(len(spec)):
      if Zspec[i] > 0.:
        if (T / Zspec[i] / Zspec[i]) <= 320000:
          gff = 0.79464 + 0.1243 * np.log10(T / Zspec[i] / Zspec[i])
        else:
          gff = 2.13164 - 0.1240 * np.log10(T / Zspec[i] / Zspec[i])
        
        LFF += 1.426e-27 * Zspec[i] * Zspec[i] * T**0.5 * gff * ne * spec[i]
    
    print('FF = ', gfree(T) * ne * cFree, LFF)
