...
def Lambda(T, nHI, nHII, nHm, nHeI, nHeII, nHeIII, nCI, nCII, nCIII, nCIV, nCV, 
  ...
  ne = (nHII - nHm + (nHeII + 2.0 * nHeIII) + 1 * nCII + 2 * nCIII + 3 * nCIV
     + 4 * nCV + 5 * nCVI + 6 * nCVII - nCm)
...
def func(t, y):
  ...
  dnHI_dt = (10**k2(Tx) * nHII * ne 
            -10**k1(Tx) * nHI * ne 
            +10**grain_rec_HII_to_HI * nHII * ne
            -10**km1(Tx) * nHI * ne
            +10**km2(Tx) * nHm * nHI #---> This is why it is POSITIVE: (Hm + HI ---> HI + HI + e) We DONOT lose HI - we create it !!!!!!!
            +10**km3(Tx) * nHm * ne
            +10**km4(Tx) * nHm * nHII
            +10**km5(Tx) * nHeII * nHm
            +10**km6(Tx) * nHeI * nHII
            -10**km7(Tx) * nHeII * nHI
            -10**km8(Tx) * nHeIII * nHI
            )
  
  dnHII_dt = (10**k1(Tx) * nHI * ne 
             -10**k2(Tx) * nHII * ne 
             -10**grain_rec_HII_to_HI * nHII * ne
             -10**km4(Tx) * nHm * nHII
             -10**km6(Tx) * nHeI * nHII
