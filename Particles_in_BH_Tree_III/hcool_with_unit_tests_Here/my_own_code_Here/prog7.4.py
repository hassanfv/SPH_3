...
# In this version (i.e. prog7.4.py) I used technique in Draine book section 27.3.1 for recombination cooling rates (Thanks to T. Grassi).
# In this version (i.e. prog7.3.py) I replaced y = Y / 4.0 / (1.0 - Y) with nHe/nH ratio from Asplund et al. (2009)!!
# In this version (i.e. prog7.2.py) we also include ionic ratios from CHIMEs as dotted lines!
...
#----- gamma_numeric
def gamma_numeric(alpha, T): # Only scalar T
  dT = 10.0
  T1 = T - dT
  T2 = T + dT
  return (np.log(alpha(T2)) - np.log(alpha(T1))) / (np.log(T2) - np.log(T1)) - 0.5



#----- getKE_numeric
def getKE_numeric(alpha, T):
  gam = gamma_numeric(alpha, T)
  return (2.0 + gam) * kB * T
...
def g2(T):
  ...
  g2_val = k2(T) * getKE_numeric(k2, T)
...
def g8(T):
  ...
  g8_val = k5(T) * getKE_numeric(k5, T)
...
def g9(T):
  ...
  g9_val = k6(T) * getKE_numeric(k6, T)
...
def g10(T):
  ...
  g10_val = k7(T) * getKE_numeric(k7, T)
