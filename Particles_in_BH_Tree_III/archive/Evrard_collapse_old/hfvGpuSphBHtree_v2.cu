...
//--------------------------------------------
  ...
  gpuErrchk(cudaMemset(d_mutexX, 0, sizeof(int))); // HHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH
  ...
  //float maxVal = fmax(h_leftX, fmax(h_rightX, fmax(h_bottomX, fmax(h_topX, fmax(h_frontX, h_backX)))));



  // FIND THE MAX FROM ABOVE AND REPLACE IT WITH maxRange..... DO IT ALSO FOR MAIN LOOP AND REMOVE maxCoord kernel!!!!!
  ...
  cudaMemcpy(d_maxArr, maxArr, gridSize * sizeof(float), cudaMemcpyHostToDevice);
  
  maxCoord<<<gridSize, blockSize, blockSize * sizeof(float)>>>(d_x, d_y, d_z, d_Typ, d_maxArr, N);
  cudaDeviceSynchronize();
  ...
  */
  ...
  gpuErrchk(cudaMemset(d_mutex, 0, sizeof(int))); // HHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH
